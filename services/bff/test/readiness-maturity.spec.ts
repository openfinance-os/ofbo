...
describe('product maturity', () => {
  ...
  it('derives one port row per catalog port — every sim ready, every enterprise a stub', () => {
    const m = getMaturity()
    ...
    expect(m.ports.every((p) => p.enterprise_status === 'stub')).toBe(true)
    expect(m.summary.sim_adapters_ready).toBe(PORTS.length)
    expect(m.summary.enterprise_adapters_remaining).toBe(PORTS.length)
...
describe('GET /public/readiness/maturity', () => {
  it('serves the maturity summary publicly (no auth, no FAPI header)', async () => {
    ...
    expect(body.data.summary.enterprise_adapters_remaining).toBe(9)
