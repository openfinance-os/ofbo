...
function fail(c: Context, e: unknown): Response {
  ...
  if (e instanceof RespondentDisputeError) {
    return c.json(errorEnvelope(e.code, e.message, 'See the respondent-dispute contract (BACKOFFICE-75).', DOCS_BASE), e.status as ContentfulStatusCode)
  }
  throw e
}
...
export function respondentDisputeRoutes(service: RespondentDisputeService, idempotency: IdempotencyStore): Record<string, Handler> {
  ...
  const withIdempotency =
    ...
    async (c, params) => {
      ...
      const cacheKey = `${routeKey}|${params.respondent_dispute_id ?? ''}|${c.get('principal').subject}|${key}`
