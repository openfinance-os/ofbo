...
function fail(c: Context, e: unknown): Response {
  ...
  if (e instanceof TrustFrameworkError) {
    return c.json(errorEnvelope(e.code, e.message, 'See the Trust Framework participant contract (BACKOFFICE-74).', DOCS_BASE), e.status as ContentfulStatusCode)
  }
  throw e
}
...
export function trustFrameworkRoutes(service: TrustFrameworkService, idempotency: IdempotencyStore): Record<string, Handler> {
  const withIdempotency =
    ...
    async (c, params) => {
      ...
      const cacheKey = `${routeKey}|${params.participant_id ?? ''}|${c.get('principal').subject}|${key}`
