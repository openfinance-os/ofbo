...
|---|---|---|
| **M0 — Repo foundation** | Monorepo scaffold, CI with gates Q1–Q3, OpenAPI client generation + failing contract stubs for all 49 paths, relational schema (9 tables, RLS, INSERT-only audit), synthetic test data + seeded demo dataset, port interfaces defined with simulator stubs | CI green on empty implementations; schema applied to the free-tier database; zero real PII anywhere |
