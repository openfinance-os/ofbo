...
- **UI/UX reference (binding):** build **every** portal screen — anywhere, not just the UI-00..09 track — against the **Open Finance Back Office Design System** ("Institutional Blue"), mirrored in `apps/portal/design/`. Division of truth: **`apps/portal/design/tokens.ts` = design tokens** (the source, not a mirror of an external tool — ADR 0026 retired Stitch); **`specs/backoffice-openapi.yaml` = behaviour + data**. Screens are token-only (no raw hex/px), OpenAPI-bound (no mock values on the wire), DEMO-bannered, persona scope-gated, zero PII, four-eyes via `202` + `approval_request` (never inline). Cite the design-system **component or pattern** in the PR and **attach a screenshot of the built screen** — with no external reference to diff against, the rendered review *is* the check. Three gates enforce the rest: `design-conformance.spec.ts` (token-only), the AA contrast assertions in `design-tokens.spec.ts`, and the four-state status set. If a needed component isn't in the system, add it there first — never invent it in a screen.
...
- Every port has TWO adapter implementations behind one interface, selected by config (`DEPLOY_PROFILE=demo|enterprise`) — application core code NEVER branches on profile (ESLint enforces this: reading `DEPLOY_PROFILE` outside `packages/ports` is a lint error). Layout as built: the demo adapters live together in `packages/ports/src/adapters/sim.ts`; each enterprise adapter is its own vendor-named file under `packages/ports/src/adapters/enterprise/` (e.g. `p2-entra.ts`, `servicenow-itsm.ts`). All ten enterprise adapters now exist and are **pre-staged ahead of M6** to ADR 0024 fidelity rung ③ — they are not stubs, and they FAIL CLOSED: an unconfigured enterprise adapter throws a config error rather than silently falling back to a fake.
...
4. Every commit and PR cites the BACKOFFICE-ID. Keep the agent build-provenance trailers (`Co-Authored-By`, `Claude-Session`) on every commit — the release evidence bundle seals them as Art. 12/17 attribution (ADR 0019).
