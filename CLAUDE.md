...
- **UI/UX reference (binding):** build **every** portal screen — anywhere, not just the UI-00..09 track — against the **Open Finance Back Office Design System** ("Institutional Blue"), mirrored in `apps/portal/design/`. Division of truth: **`apps/portal/design/tokens.ts` = design tokens** (the source, not a mirror of an external tool — ADR 0033 retired Stitch); **`specs/backoffice-openapi.yaml` = behaviour + data**. Screens are token-only (no raw hex/px), OpenAPI-bound (no mock values on the wire), DEMO-bannered, persona scope-gated, zero PII, four-eyes via `202` + `approval_request` (never inline). Cite the design-system **component or pattern** in the PR and **attach a screenshot of the built screen** — with no external reference to diff against, the rendered review *is* the check. Three gates enforce the rest: `design-conformance.spec.ts` (token-only), the AA contrast assertions in `design-tokens.spec.ts`, and the four-state status set. If a needed component isn't in the system, add it there first — never invent it in a screen.
